...
/**
 * Converts a parsed {@link Sentence} to Conjunctive Normal Form (a conjunction of clauses, each
...
public final class CNFConverter {
  ...
  private static Sentence eliminateImplications(Sentence s) {
    ...
    switch (cs.connective()) {
      ...
      case AND:
      case OR:
        return ComplexSentence.binary(cs.connective(), eliminateImplications(cs.left()),
  ...
  private static Sentence toNNF(Sentence s) {
    ...
    if (cs.connective() == Connective.NOT) {
      ...
      return switch (cchild.connective()) {
        case NOT -> toNNF(cchild.left());                 // !!x -> x
        case AND ->                                         // !(a & b) -> !a | !b
            ComplexSentence.binary(Connective.OR, toNNF(ComplexSentence.not(cchild.left())),
                toNNF(ComplexSentence.not(cchild.right())));
        case OR ->                                          // !(a | b) -> !a & !b
            ComplexSentence.binary(Connective.AND, toNNF(ComplexSentence.not(cchild.left())),
                toNNF(ComplexSentence.not(cchild.right())));
        default -> throw new IllegalStateException(
            "Implications/IFF should already be eliminated, got: " + cchild.connective());
      };
    }
    return ComplexSentence.binary(cs.connective(), toNNF(cs.left()), toNNF(cs.right()));
  }
